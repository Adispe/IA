...
def load_data_train(image_path, mask_path, sample_ids, batch_size):
    ...
    for batch_idx in tqdm(range(num_batches), desc="Loading Data Batches"):
        ...
        images_list.append(tf.convert_to_tensor(batch_images))
        masks_list.append(tf.convert_to_tensor(batch_masks))
        tf.keras.backend.clear_session()


    images = tf.concat(images_list, axis=0)
    masks = tf.concat(masks_list, axis=0)
...
images_train, masks_train = load_data_train(image_path_train, mask_path_train, sample_ids_list_train, BATCH_SIZE)
...
model.save(r"/Users/anton/Desktop/UNET_Skyscan.h5")
